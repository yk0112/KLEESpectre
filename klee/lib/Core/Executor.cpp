...
cl::OptionCategory SpecCat("Speculative execution options",
                           "These options impact the specualtive paths "
                           "exploring and the cache modeling");
...
cl::opt<bool> EnableSpeculativeExecution(
    "enable-speculative", cl::init(false),
    cl::desc("Enable Speculative exeuction modeling (default=off)."),
    cl::cat(SpecCat));

cl::opt<bool>
    EnableCacheModeling("enable-cachemodel", cl::init(false),
                        cl::desc("Enable Cache modeling (default=off)."),
                        cl::cat(SpecCat));

cl::opt<unsigned> MaxSew("max-sew", cl::desc("Maximum SEW (default=10)"),
                         cl::init(10), cl::cat(SpecCat));

cl::opt<unsigned> CacheWays("cache-ways", cl::desc("Cache ways (default=2)"),
                            cl::init(2), cl::cat(SpecCat));

cl::opt<unsigned> CacheLineSize("cache-line-size",
                                cl::desc("Cache line size (default=64)"),
                                cl::init(64), cl::cat(SpecCat));

cl::opt<unsigned> CacheSets("cache-sets", cl::desc("Cache sets (default=256)"),
                            cl::init(256), cl::cat(SpecCat));
...
cl::opt<bool> OnlyDetectMarkedSecret(
    ...
    cl::desc("Only detect marked secret (default=false)"), cl::init(false),
    cl::cat(SpecCat));
...
Executor::Executor(LLVMContext &ctx, const InterpreterOptions &opts,
  ...
  if (ispecEnabled) {
    maxSEW = MaxSew;
    specOrder = SpeculativeOrder;
    klee_warning("@Speculative execution modeling is enable! maxSEW=%d",
                 maxSEW);
  }
...
void Executor::initializeGlobals(ExecutionState &state) {
  ...
  for (auto i = m->alias_begin(), ie = m->alias_end(); i != ie; ++i) {
    ...
    globalAddresses.insert(
        std::make_pair(&*i, evalConstant(alias->getAliasee())));
  }
...
const Cell &Executor::eval(KInstruction *ki, unsigned index,
  ...
  if (vnumber < 0) {
    ...
    // llvm::errs() << sf.locals[index].getvalue();
...
void Executor::executeInstruction(ExecutionState &state, KInstruction *ki) {
  ...
  klee_message("\n================================");
  if (state.isSpeculative) {
      klee_message("SS%ld,  inst: %d", state.tag, ki->info->assemblyLine);
  } else {
  ...
  */
  ...
  case Instruction::Br: {
    ...
    if (bi->isUnconditional()) {
      ...
      if (ki->info->isInUserCode && statsTracker &&
...
void Executor::run(ExecutionState &initialState) {
  ...
  while (!states.empty() && !haltExecution) {
    // for (std::set<ExecutionState*>::iterator it=states.begin();
    // it!=states.end(); ++it) { ExecutionState *es = *it; if
    // (es->isSpeculative)
...
void Executor::terminateSpecState(ExecutionState &state) {
  ...
  // klee_message("Terminate a speculative state: s%ld.%d", state.tag,
  // state.specInstCount); terminateState(state); klee_message("Terminate
  // Speculative state: %ld", state.tag);
  //
  // assert(false);
...
ref<Expr> Executor::createSensitiveValue(Expr::Width type) {
  ...
  ref<Expr> temp =
      ReadExpr::create(updates, ConstantExpr::create(0, Expr::Int32));
  return temp;
}
    ...
    */
    ...
    if (state.isSpeculative) {
      if (address.get()->getSensitive()) {
        ...
        // LeakageKind::inConstraint, isConst);
      }
    }
    /*
           if (isCacheEnabled) {
